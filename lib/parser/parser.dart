...
class BeancountParserDefinition extends BeancountGrammarDefinition {
  ...
  Parser commodity() => super.commodity().map((each) {
        final e = each as List;
        return Commodity(
          date: e.first,
          currency: e.elementAt(2),
          metadata: e.elementAt(4),
        );
      });
  Parser event() => super.event().map((each) {
        final e = each as List;
        return Event(
          date: e.first,
          key: e.elementAt(2),
  ...
  Parser pad() => super.pad().map((each) {
        final e = each as List;
        return Pad(
          date: e.first,
          account: e.elementAt(2),
          padAccount: e.elementAt(3),
        );
      });
}
